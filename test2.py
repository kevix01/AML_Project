...
def faithful_diffattack(image_path, text_prompt, num_steps=30, lr=0.02, alpha=2.0, beta=0.5,
                        t_samples=None, image_size=256):
    """
    Parametri aggiuntivi:
    - t_samples: lista di interi (timestep) da utilizzare per il calcolo delle loss.
                 Default [150, 400] se non specificato.
    - image_size: int (per immagine quadrata) o tupla (larghezza, altezza).
                 Default 256.
    """
    if t_samples is None:
        t_samples = [150, 400]   # default per compatibilità
    if not t_samples:
        raise ValueError("t_samples non può essere vuoto.")
    ...
    # 1. PREPARAZIONE (Risoluzione parametrizzata)
    ...
    print(f"[*] Estrazione mappe di attenzione originali su timestep: {t_samples}...")
    clean_maps_sa = {}
    clean_maps_ca = {}
...
if __name__ == "__main__":
    ...
    # 3. Lancio DiffAttack (Fedele) - esempio con parametri personalizzati
    print("\n--- AVVIO DIFFATTACK (Pixel Space + SA/CA Control) ---")
    # Esempio: 3 timestep, risoluzione 320, learning rate 0.03
    ...
    if len(res_adv[0].boxes) < len(res_orig[0].boxes):
        ...
        print("\nAttacco resistito da YOLO. Prova a incrementare il lr oppure a ridurre alpha.")